...
//    this->setWindowState(Qt::windows);
    //设置窗口出现位置
    QDesktopWidget desktopWidget;
    setGeometry((desktopWidget.width()-this->width())/2, (desktopWidget.height()-this->height())/2, 969, 639);
    //渐变出现
    ...
    //多媒体信息改变
    connect(musicList->player, SIGNAL(durationChanged(qint64)),             //多媒体时长信息出现，处理歌词
            this, SLOT(durationChanged()));
    connect(musicList->player, SIGNAL(positionChanged(qint64)),             //播放进度 改变 更新歌词
...
//    }
    ...
    connect(bottomGroupbox, SIGNAL(lrc_click(bool)),                        //显示/隐藏 桌面歌词
            this, SLOT(setLrcShown(bool)));
    connect(bottomGroupbox, SIGNAL(showVolumn(QPoint)),                     //音量     处理
            this, SLOT(showVolumn(QPoint)));
    connect(bottomGroupbox, SIGNAL(showPlayModle(QPoint)),                  //单击 “播放模式”， 显示/隐藏 “播放模式”窗口
            this, SLOT(showPlayModle(QPoint)));                             //................设置playModel的当前音量值
    ...
    //播放列表控制
//    connect(musicList, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)),  //双击 播放歌曲
//            this, SLOT(itemPlay()));
...
//    connect(titleGroupBox, SIGNAL(clickGlobalMusic()),                    //单击 本地音乐
//            this, SLOT(clickedGlobalMusic()));
//    connect(titleGroupBox, SIGNAL(clickInternetMusic()),                  //单击 网络歌曲
...
//    connect(player, SIGNAL(audioAvailableChanged(bool)),                //未知信号
...
//    layout_middleLeft->addWidget(ln_search, 453, 0, 26, 276);
...
//                "}"
    ...
    tbn_search ->setStyleSheet("QToolButton{"
                                    "border-image: url(:/Images/search.png)"
                               "}"
                               );

    //信号槽关联
    connect(cob_addMusic, SIGNAL(activated(int)),
            this, SLOT(addMusicFile(int)));
    connect(tbn_createList, SIGNAL(clicked()),
            this, SLOT(create_MusicList()));
    connect(tbn_search, SIGNAL(clicked()),
            this, SLOT(tbn_search_clicked()));
}
...
void DT_Music::volumn_widget()
{
    ...
    lab_volumnFrame->hide();
    lab_volumnFrame->installEventFilter(this);
//    lab_volumnFrame->setStyleSheet("border: 1px solid white;");
//    slider_volumn = new QSlider(Qt::Vertical, this);              //初始化 音量控件 并隐藏
    slider_volumn = new QSlider(Qt::Vertical, lab_volumnFrame);
//    slider_volumn ->hide();                                       //..................
...
//    lab_volumnValue->resize(25, 10);
//    slider_volumn->resize(8, 80);
...
/*
//单击 本地音乐 处理
void DT_Music::clickedGlobalMusic()
{
//    toolInternet ->setHidden(true);
//    internetWidget ->setHidden(true);
    toolGlobal ->setVisible(true);
    lrcView ->setVisible(true);
}

//单击 网络歌曲 处理
void DT_Music::clickedInternetMusic()
{
    toolGlobal ->setHidden(true);
    lrcView ->setHidden(true);
    toolInternet ->setVisible(true);
    internetWidget ->setVisible(true);
}
*/
...
void DT_Music::ShowVideoPlayer()
{
    ...
    videoPlayer->show();
    close();
    connect(videoPlayer, SIGNAL(switchMusicPlayer()),
            this, SLOT(switchMusicPlayer()));
}
...
void DT_Music::setTheme(int theme)
{
    setStyleSheet(tr(
                      "#centralWidget{"
                            "border-image: url(:/Images/bg%1.jpg);"
                      "}"
                  ).arg(theme)
                 );
}
